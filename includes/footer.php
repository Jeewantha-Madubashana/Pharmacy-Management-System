  <!-- footer -->
        <div class="footerw3-agile">
            <div class="container">
                <div class="footer-row">
                    <div class="col-md-4 footer-grids">
                        <h3>
                            Navigation
                        </h3>
                        <ul>
                            <li>
                                <a href="about.php">
                                    <i aria-hidden="true" class="fa fa-arrow-right">
                                    </i>
                                    About us
                                </a>
                            </li>
                            <li>
                                <a href="contact.php">
                                    <i aria-hidden="true" class="fa fa-arrow-right">
                                    </i>
                                    Contact Us
                                </a>
                            </li>
                        </ul>
                    </div>
                    <div class="col-md-4 footer-grids">
                        <h3>
                            Medicine Order
                        </h3>
                        <ul>
                            <li>
                                24 Hours Open (All Registered Pharmacy)
                            </li>
                            <li>
                                In The Google Map, <br>
                                Only Registered Pharmacy have been included.
                                
                            </li>
                            
                        </ul>
                    </div>
                    <div class="col-md-4 footer-grids">
                        <h2 style="color: white;font-size: 35px;">
                                Medi At The Door
                        </h2>
                        <p>
                            Online Medicine Order <br>
                            Medicine Distribution for all branches <br>
                            Choose Medicine From The Pharmacy <br>
                            Order medicine and we can provide medicine quickly<br>
                        </p>
                    </div>
                    <div class="clearfix">
                    </div>
                </div>
            </div>
        </div>
        <div class="footer-bottom">
            <div class="container">
                <div class="footer-left">
                    <p>
                        <!-- © 2017 Medi Plus . All rights reserved | Design by
                        <a href="http://w3layouts.com">
                            W3layouts
                        </a> -->

                        Copyright © 2018 Online Medicine Order. All rights reserved.
                    </p>
                </div>
                <div class="footer-right">
                    <ul class="w3-agileitsicons">
                        <li>
                            <a href="#">
                                <i aria-hidden="true" class="fa fa-facebook">
                                </i>
                            </a>
                        </li>
                        <li>
                            <a class="twt" href="#">
                                <i aria-hidden="true" class="fa fa-twitter">
                                </i>
                            </a>
                        </li>
                        <li>
                            <a class="drbl" href="#">
                                <i aria-hidden="true" class="fa fa-linkedin">
                                </i>
                            </a>
                        </li>
                        <li>
                            <a class="be" href="#">
                                <i aria-hidden="true" class="fa fa-dribbble">
                                </i>
                            </a>
                        </li>
                    </ul>
                </div>
                <script>
                    $(function () {
              $('[data-toggle="tooltip"]').tooltip()
            })
                </script>
            </div>
        </div>
        <!-- //footer -->
        <!-- start-smooth-scrolling -->
        <script type="text/javascript">
            jQuery(document).ready(function($) {
                $(".scroll").click(function(event){
                    event.preventDefault();

            $('html,body').animate({scrollTop:$(this.hash).offset().top},1000);
                });
            });
        </script>
        <!-- //end-smooth-scrolling -->
        <script src="js/numscroller-1.0.js" type="text/javascript">
        </script>
        <!-- Bootstrap core JavaScript
    ================================================== -->
        <!-- Placed at the end of the document so the pages load faster -->
        <script src="js/bootstrap.js">
        </script>
    </body>
</html>
